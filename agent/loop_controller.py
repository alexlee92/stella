...
class LoopController:
    ...
    def decision_loop_detected(self, action: str, args: dict) -> bool:
        ...
        history = self.agent._decision_signatures

        if action in self._READ_ONLY_ACTIONS:
            # Répétition consécutive immédiate = vraie boucle (l'agent vient juste de le faire)
            if history and history[-1] == sig:
                return True
            # Répétition non-consécutive : seuil relevé à 3 pour éviter les faux positifs
            # (ex: read A → propose_edit → read A est LÉGAL)
            if history[-6:].count(sig) >= 3:
                return True
            return False

        # Pour les actions non-lecture : logique originale (même décision ≥2 dans les 4 dernières)
        if history[-4:].count(sig) >= 2:
            return True
