...
# I1-fix: aligné sur action_schema.py — toutes les actions récentes présentes
_ALLOWED_ARGS_PER_ACTION: Dict[str, set] = {
    ...
    "read_schema": {"files"},
    "plan_files": {"description"},
    "mcp_call": {"server", "method", "params"},
    "delete_file": {"path"},
    "move_file": {"src", "dst"},
...
def normalize_decision(decision: dict) -> dict:
    # B1-fix: LLMs sometimes wrap a single decision in an array [{"action":...}]
    # Unwrap single-element lists that contain a decision dict.
...
def coerce_decision(goal: str, decision: dict) -> dict:
    ...
    # I6-fix: ne plus vider systématiquement "contains" — le planner peut filtrer
    # On preserve le filtre "contains" s'il est présent et raisonnable.
