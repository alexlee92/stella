...
def goto_definition(path: str, symbol: str) -> List[dict]:
    ...
    try:
        ...
        return (
            results if results else [{"error": f"no definition found for '{symbol}'"}]
        )
    ...
    try:
        ...
        for r in refs:
            ...
            if r.module_path and os.path.isfile(str(r.module_path)):
                try:
                    lines = open(
                        str(r.module_path), "r", encoding="utf-8", errors="ignore"
                    ).readlines()
        ...
        return (
            results if results else [{"error": f"no references found for '{symbol}'"}]
        )
    except Exception as e:
        return [{"error": f"jedi error: {e}"}]
