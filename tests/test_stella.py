...
class TestDetectTaskType:
    ...
    def test_optimize_english(self):
        assert (
            self._detect("optimize the performance of this function") == "optimization"
        )
    ...
    def test_refactor_french(self):
        assert (
            self._detect("refactoriser cette classe pour améliorer la lisibilité")
            == "refactor"
        )
    ...
    def test_backend_french(self):
        assert (
            self._detect("créer une route API dans Flask avec base de données")
            == "backend"
        )
