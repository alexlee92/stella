...
from stella_ai import prompts, utils
...
class GitRepo:
    repo = None
    stella_ignore_file = None
    stella_ignore_spec = None
    stella_ignore_ts = 0
    stella_ignore_last_check = 0
    subtree_only = False
    ignore_file_cache = {}
    git_repo_error = None
    ...
    def __init__(
        ...
        if stella_ignore_file:
            self.stella_ignore_file = Path(stella_ignore_file)

    def commit(self, fnames=None, context=None, message=None, stella_edits=False, coder=None):
        """
        ...
        Args:
            ...
            stella_edits (bool, optional): Whether the changes were made by Stella. Defaults to False.
        ...
        Stella generated the changes (`stella_edits`).
        ...
        - stella_edits=True: Changes were generated by Stella (LLM).
        - stella_edits=False: Commit is user-driven (e.g., /commit manually staged changes).
        ...
        Flags:
        - --attribute-author: Modify Author name to "User Name (stella)".
        - --attribute-committer: Modify Committer name to "User Name (stella)".
        - --attribute-co-authored-by: Add
          "Co-authored-by: stella (<model>) <stella@stella.chat>" trailer to commit message.

        Behavior Summary:

        1. When stella_edits = True (AI Changes):
        ...
        2. When stella_edits = False (User Changes):
           - --attribute-co-authored-by is IGNORED (trailer never added).
           - Author name is NEVER modified (--attribute-author ignored).
           - Committer name IS modified by default (implicit True, as Stella runs `git commit`).
           - EXCEPTION: If --attribute-committer is EXPLICITLY False, the name is NOT modified.

        Resulting Scenarios:
        - Standard AI edit (defaults): Co-authored-by=False -> Author=You(stella),
          Committer=You(stella)
        ...
        - AI edit with Co-authored-by + Explicit Author: Co-authored-by=True,
          --attribute-author -> Author=You(stella), Committer=You, Trailer added
        - User commit (defaults): stella_edits=False -> Author=You, Committer=You(stella)
        - User commit with explicit no-committer: stella_edits=False,
        ...
        prefix_commit_message = stella_edits and (
        ...
        if stella_edits and attribute_co_authored_by:
            model_name = "unknown-model"
            if coder and hasattr(coder, "main_model") and coder.main_model.name:
                model_name = coder.main_model.name
            commit_message_trailer = f"\n\nCo-authored-by: stella ({model_name}) <stella@stella.chat>"

        # Determine if author/committer names should be modified
        # Author modification applies only to stella edits.
        # It's used if effective_author is True AND
        # (co-authored-by is False OR author was explicitly set).
        use_attribute_author = (
            stella_edits and effective_author and (not attribute_co_authored_by or author_explicit)
        )

        # Committer modification applies regardless of stella_edits (based on tests).
        # It's used if effective_committer is True AND
        # (it's not an stella edit with co-authored-by OR committer was explicitly set).
        use_attribute_committer = effective_committer and (
            not (stella_edits and attribute_co_authored_by) or committer_explicit
        )
        ...
        if prefix_commit_message:
            commit_message = "stella: " + commit_message
        ...
        committer_name = f"{original_user_name} (stella)"
    ...
    def refresh_stella_ignore(self):
        if not self.stella_ignore_file:
            return

        current_time = time.time()
        if current_time - self.stella_ignore_last_check < 1:
            return

        self.stella_ignore_last_check = current_time

        if not self.stella_ignore_file.is_file():
            return

        mtime = self.stella_ignore_file.stat().st_mtime
        if mtime != self.stella_ignore_ts:
            self.stella_ignore_ts = mtime
            self.ignore_file_cache = {}
            lines = self.stella_ignore_file.read_text().splitlines()
            self.stella_ignore_spec = pathspec.PathSpec.from_lines(
    ...
    def ignored_file_raw(self, fname):
        ...
        if not self.stella_ignore_file or not self.stella_ignore_file.is_file():
            return False
        ...
        return self.stella_ignore_spec.match_file(fname)
