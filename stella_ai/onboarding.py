...
def offer_openrouter_oauth(io, analytics):
    ...
    """
    io.tool_warning("Flux OpenRouter/OAuth desactive (aucun lien web n'est ouvert).")
    analytics.event("oauth_flow_disabled", provider="openrouter")
    return False
...
def start_openrouter_oauth_flow(io, analytics):
    ...
    callback_url = f"http://localhost:{port}/callback/stella"
    auth_code = None
    server_error = None
    ...
    class OAuthCallbackHandler(http.server.SimpleHTTPRequestHandler):
        def do_GET(self):
            ...
            if parsed_path.path == "/callback/stella":
                ...
                if "code" in query_params:
                    ...
                    self.wfile.write(
                        ...
                        b"<p>Stella has received the authentication code. "
                    ...
                    # Redirect to stella website if 'code' is missing (e.g., user visited manually)
    ...
    io.tool_output("\nPlease open this URL in your browser to connect Stella with OpenRouter:")
    io.tool_output()
    print(auth_url)
    ...
    if api_key:
        ...
        try:
            config_dir = os.path.expanduser("~/.stella")
            os.makedirs(config_dir, exist_ok=True)
            key_file = os.path.join(config_dir, "oauth-keys.env")
            with open(key_file, "a", encoding="utf-8") as f:
                f.write(f'OPENROUTER_API_KEY="{api_key}"\n')

            io.tool_warning("Stella will load the OpenRouter key automatically in future sessions.")
            io.tool_output()
