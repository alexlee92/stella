...
from stella_ai import __version__
from stella_ai.args_formatter import (
    DotEnvFormatter,
    MarkdownHelpFormatter,
    YamlHelpFormatter,
)
from stella_ai.deprecated import add_deprecated_model_args

from .dump import dump  # noqa: F401


def resolve_stellaignore_path(path_str, git_root=None):
    path = Path(path_str)
    if path.is_absolute():
        return str(path)
    elif git_root:
...
def get_parser(default_config_files, git_root):
    parser = configargparse.ArgumentParser(
        description="stella is AI pair programming in your terminal",
        add_config_file_help=True,
        default_config_files=default_config_files,
        config_file_parser_class=configargparse.YAMLConfigFileParser,
        auto_env_var_prefix="STELLA_",
    )
    ...
    from stella_ai import coders as _stella_coders

    edit_format_choices = sorted(
        {
            c.edit_format
            for c in _stella_coders.__all__
    ...
    group.add_argument(
        ...
        help="Specify a file with stella model settings for unknown models",
    ).complete = shtab.FILE
    group.add_argument(
        "--model-metadata-file",
    ...
    default_input_history_file = (
        os.path.join(git_root, ".stella.input.history") if git_root else ".stella.input.history"
    )
    default_chat_history_file = (
        os.path.join(git_root, ".stella.chat.history.md") if git_root else ".stella.chat.history.md"
    )
    group.add_argument(
        "--input-history-file",
    ...
    group.add_argument(
        ...
        default=None,
        help="Log the conversation with the LLM to this file (for example, .stella.llm.history)",
    ).complete = shtab.FILE
    ...
    group.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Enable/disable adding .stella* to .gitignore (default: True)",
    )
    group.add_argument(
        "--add-gitignore-files",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Enable/disable the addition of files listed in .gitignore to Stella's editing scope.",
    )
    default_stellaignore_file = (
        os.path.join(git_root, ".stellaignore") if git_root else ".stellaignore"
    )

    group.add_argument(
        "--stellaignore",
        metavar="STELLAIGNORE",
        type=lambda path_str: resolve_stellaignore_path(path_str, git_root),
        default=default_stellaignore_file,
        help="Specify the stella ignore file (default: .stellaignore in git root)",
    ).complete = shtab.FILE
    group.add_argument(
        "--subtree-only",
        action="store_true",
    ...
    group.add_argument(
        ...
        default=None,
        help=(
            "Attribute stella code changes in the git author name (default: True). If explicitly set"
    ...
    group.add_argument(
        ...
        default=None,
        help=(
            "Attribute stella commits in the git committer name (default: True). If explicitly set"
            " to True, overrides --attribute-co-authored-by precedence for stella edits."
        ),
    )
    group.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Prefix commit messages with 'stella: ' if stella authored the changes (default: False)",
    )
    group.add_argument(
        "--attribute-commit-message-committer",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Prefix all commit messages with 'stella: ' (default: False)",
    )
    group.add_argument(
        ...
        default=True,
        help=(
            "Attribute stella edits using the Co-authored-by trailer in the commit message"
    ...
    group.add_argument(
        "--check-update",
        action=argparse.BooleanOptionalAction,
        help="Check for new stella versions on launch",
        default=True,
    )
    group.add_argument(
        "--show-release-notes",
    ...
    group.add_argument(
        ...
        "--update",
        action="store_true",
        help="Upgrade stella to the latest version from PyPI",
        default=False,
    )
    group.add_argument(
        "--version",
    ...
    group.add_argument(
        ...
        "--browser",
        action=argparse.BooleanOptionalAction,
        help="Run stella in your browser (default: False)",
        default=False,
    )
    group.add_argument(
        "--copy-paste",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Enable automatic copy/paste of chat between stella and web UI (default: False)",
    )
    group.add_argument(
        "--apply",
        metavar="FILE",
    ...
    group.add_argument(
        ...
        metavar="CONFIG_FILE",
        help=(
            "Specify the config file (default: search for .stella.conf.yml in git root, cwd"
            " or home directory)"
        ),
    ).complete = shtab.FILE
    ...
    group.add_argument(
        ...
        help=(
            ...
            f" {', '.join(supported_shells_list)}. Example: stella --shell-completions bash"
        ),
    )
    ...
    if command == "md":
        ...
        if len(sys.argv) > 2:
            ...
            if shell not in shtab.SUPPORTED_SHELLS:
                ...
                sys.exit(1)
            parser = get_parser([], None)
            parser.prog = "stella"  # Set the program name on the parser
