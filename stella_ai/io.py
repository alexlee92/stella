...
from stella_ai.mdstream import MarkdownStream
...
# Constants
NOTIFICATION_MESSAGE = "Stella is waiting for your input"
...
class InputOutput:
    ...
    _FR_REPLACEMENTS = {
        ...
        "Error:": "Erreur :",
        "Open URL for more info?": "Ouvrir un lien pour plus d'informations ?",
        "Open documentation url for more info?": "Ouvrir la documentation pour plus d'informations ?",
        "Open documentation URL for more info?": "Ouvrir la documentation pour plus d'informations ?",
        "For more info:": "Pour plus d'informations :",
        "Can't initialize prompt toolkit:": "Impossible d'initialiser l'interface de saisie :",
        "Detected dumb terminal, disabling fancy input and pretty output.": (
            "Terminal limite detecte, saisie avancee et affichage riche desactives."
        ),
        "Detected xterm on Windows, disabling fancy input and pretty output.": (
            "Terminal xterm detecte sous Windows, saisie avancee et affichage riche desactives."
        ),
        "Empty response received from LLM. Check your provider account?": (
            "Reponse vide du modele. Verifie ton compte fournisseur."
        ),
        "expects these environment variables": "attend ces variables d'environnement",
        "Note: You may need to restart your terminal or command prompt for `setx` to take effect.": (
            "Note : redemarre le terminal pour que `setx` soit pris en compte."
        ),
        "You can skip this check with --no-show-model-warnings": (
            "Tu peux ignorer cette verification avec --no-show-model-warnings"
        ),
        "Repo-map can't include": "La carte du repo ne peut pas inclure",
        "Has it been deleted from the file system but not from git?": (
            "Le fichier a peut-etre ete supprime du disque mais pas de git."
        ),
    }
    ...
    def __init__(
        ...
        self.append_chat_history(f"\n# stella chat started at {current_time}\n\n")

        self.prompt_session = None
        self.is_dumb_terminal = is_dumb_terminal()
        term_env = (os.environ.get("TERM") or "").lower()
        self.is_windows_xterm = os.name == "nt" and term_env.startswith("xterm")

        if self.is_dumb_terminal or self.is_windows_xterm:
            self.pretty = False
            fancy_input = False
...
{prefix} {hist}"""
    ...
    def offer_url(self, url, prompt="Open URL for more info?", allow_never=True):
        """Ouverture de liens desactivee pour Stella."""
        _ = (url, allow_never)
        if prompt:
            self.tool_warning(self._localize_and_sanitize(prompt))
        self.tool_output("Ouverture automatique de liens web desactivee.")
        return False
    ...
    def _tool_message(self, message="", strip=True, color=None):
        message = self._localize_and_sanitize(message)
        if message.strip():
            if "\n" in message:
    ...
    def tool_output(self, *messages, log_only=False, bold=False):
        messages = tuple(self._localize_and_sanitize(m) for m in messages)
        if messages:
            hist = " ".join(messages)
            hist = f"{hist.strip()}"
            self.append_chat_history(hist, linebreak=True, blockquote=True)
    ...
    def get_default_notification_command(self):
        ...
        if system == "Darwin":  # macOS
            ...
            if shutil.which("terminal-notifier"):
                return f"terminal-notifier -title 'Stella' -message '{NOTIFICATION_MESSAGE}'"
            # Fall back to osascript
            return (
                f'osascript -e \'display notification "{NOTIFICATION_MESSAGE}" with title "Stella"\''
            )
        elif system == "Linux":
            ...
            for cmd in ["notify-send", "zenity"]:
                if shutil.which(cmd):
                    if cmd == "notify-send":
                        return f"notify-send 'Stella' '{NOTIFICATION_MESSAGE}'"
                    elif cmd == "zenity":
    ...
    def _localize_and_sanitize(self, value):
        text = str(value) if value is not None else ""
        text = self._URL_RE.sub("[lien masque]", text)
        for src, dst in self._FR_REPLACEMENTS.items():
