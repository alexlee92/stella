...
from stella_ai import __version__, models, urls, utils
from stella_ai.analytics import Analytics
from stella_ai.args import get_parser
from stella_ai.coders import Coder
from stella_ai.coders.base_coder import UnknownEditFormat
from stella_ai.commands import Commands, SwitchCoder
from stella_ai.copypaste import ClipboardWatcher
from stella_ai.deprecated import handle_deprecated_model_args
from stella_ai.format_settings import format_settings, scrub_sensitive_info
from stella_ai.history import ChatSummary
from stella_ai.io import InputOutput
from stella_ai.llm import litellm  # noqa: F401; properly init litellm on launch
from stella_ai.models import ModelSettings
from stella_ai.onboarding import offer_openrouter_oauth, select_default_model
from stella_ai.repo import ANY_GIT_ERROR, GitRepo
from stella_ai.report import report_uncaught_exceptions
from stella_ai.versioncheck import check_version, install_from_main_branch, install_upgrade
from stella_ai.watch import FileWatcher

from .dump import dump  # noqa: F401


def _translate_help_fr(text: str) -> str:
    header_map = {
        "usage:": "utilisation :",
        "options:": "options :",
        "Main model:": "Modele principal :",
        "API Keys and settings:": "Cles API et configuration :",
        "Model settings:": "Configuration des modeles :",
        "Cache settings:": "Configuration du cache :",
        "Repomap settings:": "Configuration de la carte repo :",
        "History Files:": "Fichiers d'historique :",
        "Output settings:": "Configuration de sortie :",
        "Git settings:": "Configuration Git :",
        "Fixing and committing:": "Corrections et commits :",
        "Analytics:": "Statistiques :",
        "Upgrading:": "Mises a jour :",
        "Modes:": "Modes :",
        "Voice settings:": "Configuration voix :",
        "Other settings:": "Autres options :",
        "Deprecated model settings:": "Options de modeles depreciees :",
        "stella is AI pair programming in your terminal": (
            "stella est un assistant de programmation IA dans ton terminal"
        ),
    }
    sentence_map = {
        "show this help message and exit": "affiche cette aide et quitte",
        "Show the version number and exit": "affiche la version et quitte",
        "Check for updates and return status in the exit code": (
            "verifie les mises a jour et retourne le statut via le code de sortie"
        ),
        "Run tests, fix problems found and then exit": (
            "lance les tests, corrige les problemes detectes puis quitte"
        ),
        "show release notes on first run of new version": (
            "afficher les notes de version au premier lancement d'une nouvelle version"
        ),
        "Args that start with '--' can also be set in a config file": (
            "Les options qui commencent par '--' peuvent aussi etre definies dans un fichier de configuration"
        ),
        "The config file uses YAML syntax and must represent a YAML 'mapping'": (
            "Le fichier de configuration utilise la syntaxe YAML et doit representer un 'mapping' YAML"
        ),
        "In general, command-line values override config file values which override defaults.": (
            "En general, les valeurs de la ligne de commande ont priorite sur celles du fichier de configuration, elles-memes prioritaires sur les valeurs par defaut."
        ),
        "Specify": "Specifier",
        "specify": "specifier",
        ...
        "Never ": "Ne jamais ",
        "Always ": "Toujours ",
        "Audio format for voice recording": "Format audio pour l'enregistrement vocal",
        "input device name for voice recording": "nom du peripherique d'entree pour l'enregistrement vocal",
        "language for voice using ISO 639-1 code": "langue de la voix avec code ISO 639-1",
        "for the main chat": "pour le chat principal",
        "for editor tasks": "pour les taches d'edition",
        "for unknown models": "pour les modeles inconnus",
        "in your browser": "dans ton navigateur",
        "in the terminal": "dans le terminal",
        "in git root": "dans la racine git",
        "for current session": "pour la session en cours",
        "for notifications": "pour les notifications",
        "for shell completion": "pour l'auto-completion shell",
        "chat input history file": "fichier d'historique des saisies",
        "chat history file": "fichier d'historique du chat",
        "config file": "fichier de configuration",
        "read-only file": "fichier en lecture seule",
        "Line endings to use when writing files": "fins de ligne a utiliser lors de l'ecriture",
        "default:": "defaut :",
        "(default:": "(defaut :",
        "can be used multiple times": "peut etre utilise plusieurs fois",
        "deprecated, use --model": "deprecie, utiliser --model",
    ...
    for line in text.splitlines():
        ...
        for src, dst in header_map.items():
            hline = hline.replace(src, dst)

        # lignes d'options argparse: ne jamais traduire la colonne de gauche (flags)
        m = re.match(r"^(\s{2,}\S.*?)(\s{2,})(.*)$", hline)
        if m:
            left, sep, right = m.groups()
            for src, dst in sentence_map.items():
                right = right.replace(src, dst)
            right = re.sub(r"https?://\S+", "[lien masque]", right)
            translated_lines.append(f"{left}{sep}{right}")
        else:
            for src, dst in sentence_map.items():
                hline = hline.replace(src, dst)
            hline = re.sub(r"https?://\S+", "[lien masque]", hline)
            translated_lines.append(hline)
...
def setup_git(git_root, io):
    ...
    if git_root:
        ...
        io.tool_warning(
            "You should probably run stella in your project's directory, not your home dir."
        )
        return
    elif cwd and io.confirm_ask(
        "No git repo found, create one to track stella's changes (recommended)?"
    ):
...
def check_gitignore(git_root, io, ask=True):
    ...
    try:
        ...
        if not repo.ignored(".stella"):
            patterns_to_add.append(".stella*")
...
def check_streamlit_install(io):
    return utils.check_pip_install_extra(
        io,
        "streamlit",
        "You need to install the stella browser feature",
        ["stella-chat[browser]"],
    )
...
def write_streamlit_credentials():
    ...
    # See https://github.com/Stella-AI/stella/issues/772
...
def launch_gui(args):
    ...
    # https://github.com/Stella-AI/stella/issues/2193
...
def register_models(git_root, model_settings_fname, io, verbose=False):
    model_settings_files = generate_search_path_list(
        ".stella.model.settings.yml", git_root, model_settings_fname
    )
    ...
    try:
        ...
        io.tool_error(f"Error loading stella model settings: {e}")
        return 1
...
def load_dotenv_files(git_root, dotenv_fname, encoding="utf-8"):
    ...
    oauth_keys_file = Path.home() / ".stella" / "oauth-keys.env"
...
def register_litellm_models(git_root, model_metadata_fname, io, verbose=False):
    ...
    resource_metadata = importlib_resources.files("stella_ai.resources").joinpath(
        "model-metadata.json"
    )
    model_metadata_files.append(str(resource_metadata))

    model_metadata_files += generate_search_path_list(
        ".stella.model.metadata.json", git_root, model_metadata_fname
    )
...
def sanity_check_repo(repo, io):
    ...
    if bad_ver:
        io.tool_error("Stella only works with git repos with version number 1 or 2.")
        io.tool_output("You may be able to convert your repo: git update-index --index-version=2")
        io.tool_output("Or run stella --no-git to proceed without using git.")
...
def main(argv=None, input=None, output=None, force_git_root=None, return_coder=False):
    ...
    conf_fname = Path(".stella.conf.yml")
    ...
    # Aide complete en francais
    if any(flag in argv for flag in ("-h", "--help")):
        parser = get_parser(default_config_files, git_root)
        print(_translate_help_fr(parser.format_help()))
        return 0
    ...
    # Defaults francophones
    if not args.chat_language:
        args.chat_language = "francais"
    if not args.commit_language:
        args.commit_language = "francais"
    ...
    if args.analytics is not False:
        if analytics.need_to_ask(args.analytics):
            ...
            disable = not io.confirm_ask(
                "Allow collection of anonymous analytics to help improve stella?"
            )
    ...
    if args.stellaignore:
        ignores.append(args.stellaignore)
    ...
    if args.apply:
        ...
        # For testing #2879
        # from stella_ai.coders.base_coder import all_fences
    ...
    if args.show_release_notes is True:
        io.tool_warning("Affichage des notes de version via navigateur desactive.")
    elif args.show_release_notes is None and is_first_run:
        io.tool_output("Notes de version disponibles dans la documentation locale.")
...
def is_first_run_of_new_version(io, verbose=False):
    ...
    installs_file = Path.home() / ".stella" / "installs.json"
    ...
    try:
        if is_first_run:
            ...
            try:
                ...
                io.tool_error(str(err))
                io.tool_output("Error loading required imports. Did you install stella properly?")
