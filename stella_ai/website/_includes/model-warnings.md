...
If you specify a model that stella has never heard of, you will get
this warning.
This means stella doesn't know the context window size and token costs
for that model.
Stella will use an unlimited context window and assume the model is free,
...
If stella isn't familiar with the model you've specified,
...
Stella is unable verify the environment because it doesn't know
which variables are required for the model.
If required variables are missing,
you may get errors when you attempt to chat with the model.
You can look in the [stella's LLM documentation](/docs/llms.html)
or the
