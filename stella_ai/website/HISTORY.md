...
description: Release notes and stats on stella writing its own code.
---

# Release history

Stella writes most of its own code, usually about 70-80% of the new code in each release.
These
[statistics are based on the git commit history](/docs/faq.html#how-are-the-stella-wrote-xx-of-code-stats-computed)
of the stella repo.
...
- Stella wrote 57% of the code in this release.
...
- Stella wrote 88% of the code in this release.
...
- Added `--add-gitignore-files` flag to enable adding files listed in .gitignore to Stella's editing scope, by omarcinkonis.
...
- Stella wrote 21% of the code in this release.
...
- Stella wrote 79% of the code in this release.
...
- Prevented Stella from instructing the LLM to reply in 'C' or 'POSIX' when these are detected as the system locale.
...
- The `stella scrape` command-line tool will now use Playwright for web scraping if it is available, by Jon Keys.
...
- The `stella-args` utility (or `python -m stella.args`) now defaults to printing a sample YAML configuration if no arguments are provided.
...
- Stella wrote 55% of the code in this release.
...
- Stella wrote 92% of the code in this release.

### Stella v0.81.3

- Commit messages generated by stella are no longer forced to be entirely lowercase, by Peter Hadlaw.
...
- Fix quoting of values containing '#' in the sample `stella.conf.yml`.
- Add support for Fireworks AI model 'deepseek-v3-0324', by Felix Lisczyk.
- Commit messages generated by stella are now lowercase, by Anton Ödman.
...
- Stella wrote 86% of the code in this release.
...
- Handle `FileNotFoundError` if the current working directory is deleted while stella is running.
- Fix completion menu current item color styling, by Andrey Ivanov.
- Stella wrote 87% of the code in this release.
...
- Stella wrote 93% of the code in this release.

### Stella v0.79.1

- Improved model listing to include all models in fuzzy matching, including those provided by stella (not litellm).
...
- Stella wrote 65% of the code in this release.
...
- Enhanced `--stellaignore` argument to resolve both absolute and relative paths, by mopemope.
...
- Big upgrade in [programming languages supported](https://stella.chat/docs/languages.html) by adopting [tree-sitter-language-pack](https://github.com/Goldziher/tree-sitter-language-pack/).
...
- Stella wrote 72% of the code in this release.
...
- Stella wrote 75% of the code in this release.
...
- Stella wrote 0% of the code in this release.
...
- Stella will notify you when it's completed the last request and needs your input:
  - Added [notifications when LLM responses are ready](https://stella.chat/docs/usage/notifications.html) with `--notifications` flag.
...
- Stella wrote 85% of the code in this release.
...
- Stella wrote 46% of the code in this release.
...
- Docker containers now set `HOME=/app` which is the normal project mount-point, to persist `~/.stella`.
...
- Stella wrote 77% of the code in this release.

### Stella v0.73.0

- Full support for o3-mini: `stella --model o3-mini`
...
- Stella wrote 69% of the code in this release.
...
- Stella wrote 52% of the code in this release.
...
- Stella wrote 60% of the code in this release.
...
- Stella wrote 74% of the code in this release.
...
- [Watch files](https://stella.chat/docs/usage/watch.html) improvements:
  - Use `# ... AI?` comments to trigger stella and ask questions about your code.
  - Now watches *all* files, not just certain source files.
  - Use `# AI comments`, `// AI comments`, or `-- AI comments` to give stella instructions in any text file.
- Full support for Gemini Flash 2.0 Exp:
  - `stella --model flash` or `stella --model gemini/gemini-2.0-flash-exp`
- [New `--multiline` flag and `/multiline-mode` command](https://stella.chat/docs/usage/commands.html#entering-multi-line-chat-messages) makes ENTER a soft newline and META-ENTER send the message, by @miradnanali.
- `/copy-context <instructions>` now takes optional "instructions" when [copying code context to the clipboard](https://stella.chat/docs/usage/copypaste.html#copy-stellas-code-context-to-your-clipboard-paste-into-the-web-ui).
...
- Stella wrote 68% of the code in this release.

### Stella v0.68.0

- [Stella works with LLM web chat UIs](https://stella.chat/docs/usage/copypaste.html).
  - New `--copy-paste` mode.
  - New `/copy-context` command.
- [Set API keys and other environment variables for all providers from command line or YAML conf file](https://stella.chat/docs/config/stella_conf.html#storing-llm-keys).
...
- Stella wrote 71% of the code in this release.

### Stella v0.67.0

- [Use stella in your IDE or editor](https://stella.chat/docs/usage/watch.html).
  - Run `stella --watch-files` and it will watch for instructions you add to your source files.
  - One-liner `# ...` or `// ...` comments that start or end with "AI" are instructions to stella.
  - When stella sees "AI!" it reads and follows all the instructions in AI comments.
...
- [Stella wrote 61% of the code in this release.](https://stella.chat/HISTORY.html)
...
- Stella wrote 82% of the code in this release.
...
- Added `--alias` config to define [custom model aliases](https://stella.chat/docs/config/model-aliases.html).
...
- Added [RepoMap support for Dart language](https://stella.chat/docs/languages.html) by @malkoG.
- Ask 2.5% of users if they want to opt-in to [analytics](https://stella.chat/docs/more/analytics.html).
...
- Stella wrote 81% of the code in this release.
...
- Added [`/editor` command](https://stella.chat/docs/usage/commands.html) to open system editor for writing prompts, by @thehunmonkgroup.
...
- Ask 1% of users if they want to opt-in to [analytics](https://stella.chat/docs/more/analytics.html).
- Added support for [optional multiline input tags](https://stella.chat/docs/usage/commands.html#entering-multi-line-chat-messages) with matching closing tags.
- Improved [model settings configuration](https://stella.chat/docs/config/adv-model-settings.html#global-extra-params) with support for global `extra_params` for `litellm.completion()`.
...
- Full support for Claude 3.5 Haiku
  - Scored 75% on [stella's code editing leaderboard](https://stella.chat/docs/leaderboards/).
...
- Easily apply file edits from ChatGPT, Claude or other web apps
  ...
  - Run `stella --apply-clipboard-edits file-to-edit.js`.
  - Stella will edit your file with the LLM's changes.
- Bugfix for creating new files.
- Stella wrote 84% of the code in this release.  

### Stella v0.61.0

- Load and save stella slash-commands to files:
...
- Anonymous, opt-in [analytics](https://stella.chat/docs/more/analytics.html) with no personal data sharing.
- Stella follows litellm's `supports_vision` attribute to enable image support for models.
...
- Full support for Sonnet 10/22, the new SOTA model on stella's code editing benchmark.
  - Stella uses Sonnet 10/22 by default.
...
- Stella wrote 49% of the code in this release.
...
- Renamed `--yes` to `--yes-always`.
  - Now uses `STELLA_YES_ALWAYS` env var and `yes-always:` YAML key.
...
- [Use a pair of Architect/Editor models for improved coding](https://stella.chat/2024/09/26/architect.html)
...
- Numerous bug fixes.
- Stella wrote 53% of the code in this release.

### Stella v0.57.1

- Fixed dependency conflict between stella-chat[help] and [playwright].
...
- Support for OpenAI o1 models:
  ...
  - `stella --model o1-mini`
  - `stella --model o1-preview`
...
- Stella wrote 70% of the code in this release.
...
- Numerous bug fixes for corner case crashes.
- Stella wrote 56% of the code in this release.
...
- When users launch stella in their home dir, help them find/create a repo in a subdir.
...
- New `--upgrade` switch to install latest version of stella from pypi.
- Bugfix to `--show-prompt`.
- Disabled automatic reply to the LLM on `/undo` for all models.
- Removed pager from `/web` output.
- Stella wrote 64% of the code in this release.

### Stella v0.53.0

- [Keep your prompt cache from expiring](https://stella.chat/docs/usage/caching.html#preventing-cache-expiration) with `--cache-keepalive-pings`.
...
- New `--install-main-branch` to update to the latest dev version of stella.
...
- Stella wrote 59% of the code in this release.
...
- Stella now offers to run shell commands:
...
- Show the active chat mode at stella prompt.
...
- Bugfix: properly load `.stella.models.metadata.json` data.
...
- Stella wrote 66% of the code in this release.
...
- When [scripting stella](https://stella.chat/docs/scripting.html) messages can now contain in-chat `/` commands.
- Stella in docker image now suggests the correct command to update to latest version.
...
- Enhanced commit message attribution options:
  - New `--attribute-commit-message-author` to prefix commit messages with 'stella: ' if stella authored the changes, replaces `--attribute-commit-message`.
  - New `--attribute-commit-message-committer` to prefix all commit messages with 'stella: '.
- Stella wrote 61% of the code in this release.
...
- Corrected instructions for upgrading the docker container to latest stella version.
- Removed obsolete 16k token limit on commit diffs, use per-model limits.

### Stella v0.48.0

- Performance improvements for large/mono repos.
- Added `--subtree-only` to limit stella to current directory subtree.
...
- Support for Sonnet's 8k output window.
  - [Stella already supported infinite output from Sonnet.](https://stella.chat/2024/07/01/sonnet-not-lazy.html)
- Workaround litellm bug for retrying API server errors.
- Upgraded dependencies, to pick up litellm bug fixes.
- Stella wrote 44% of the code in this release.
...
- [Commit message](https://stella.chat/docs/git.html#commit-messages) improvements:
...
- [Linting](https://stella.chat/docs/usage/lint-test.html) improvements:
...
- Improved [docker support](https://stella.chat/docs/install/docker.html):
  - Resolved permission issues when using `docker run --user xxx`.
  - New `paulgauthier/stella-full` docker image, which includes all extras.
- Switching to code and ask mode no longer summarizes the chat history.
- Added graph of stella's contribution to each release.
...
- Stella wrote 58% of the code in this release.
...
- New `/chat-mode <mode>` command to switch chat modes:
  ...
  - help: Get help about using stella (usage, config, troubleshoot).
- Add `file: CONVENTIONS.md` to `.stella.conf.yml` to always load a specific file.
...
- Stop stella from asking to add the same url to the chat multiple times.
...
- Stella wrote 45% of the code in this release.
...
- Stella is better at offering to add files to the chat on Windows.
...
- Stella wrote 42% of the code in this release.

### Stella v0.44.0

- Default pip install size reduced by 3-12x.
- Added 3 package extras, which stella will offer to install when needed:
  - `stella-chat[help]`
  - `stella-chat[browser]`
  - `stella-chat[playwright]`
...
- Stella wrote 29% of the code in this release (157/547 lines).
...
- Use `/help <question>` to [ask for help about using stella](https://stella.chat/docs/troubleshooting/support.html), customizing settings, troubleshooting, using LLMs, etc.
- Allow multiple use of `/undo`.
- All config/env/yml/json files now load from home, git root, cwd and named command line switch.
- New `$HOME/.stella/caches` dir for app-wide expendable caches.
- Default `--model-settings-file` is now `.stella.model.settings.yml`.
- Default `--model-metadata-file` is now `.stella.model.metadata.json`.
- Bugfix affecting launch with `--no-git`.
- Stella wrote 9% of the 424 lines edited in this release.
...
- [Allow Claude 3.5 Sonnet to stream back >4k tokens!](https://stella.chat/2024/07/01/sonnet-not-lazy.html)
...
- Stella now uses `claude-3-5-sonnet-20240620` by default if `ANTHROPIC_API_KEY` is set in the environment.
- [Enabled image support](https://stella.chat/docs/usage/images-urls.html) for 3.5 Sonnet and for GPT-4o & 3.5 Sonnet via OpenRouter (by @yamitzky).
- Added `--attribute-commit-message` to prefix stella's commit messages with "stella:".
...
- Options to suppress adding "(stella)" to the [git author and committer names](https://stella.chat/docs/git.html#commit-attribution).
...
- Use `--sonnet` for Claude 3.5 Sonnet, which is the top model on [stella's LLM code editing leaderboard](https://stella.chat/docs/leaderboards/#claude-35-sonnet-takes-the-top-spot).
- All `STELLA_xxx` environment variables can now be set in `.env` (by @jpshack-at-palomar).
- Use `--llm-history-file` to log raw messages sent to the LLM (by @daniel-vainsencher).
- Commit messages are no longer prefixed with "stella:". Instead the git author and committer names have "(stella)" added.

### Stella v0.38.0

- Use `--vim` for [vim keybindings](https://stella.chat/docs/usage/commands.html#vi) in the chat.
- [Add LLM metadata](https://stella.chat/docs/llms/warnings.html#specifying-context-window-size-and-token-costs) via `.stella.models.json` file (by @caseymcc).
- More detailed [error messages on token limit errors](https://stella.chat/docs/troubleshooting/token-limits.html).
...
- Stella will notice if you paste a URL into the chat, and offer to scrape it.
- Performance improvements the repo map, especially in large repos.
- Stella will not offer to add bare filenames like `make` or `run` which may just be words.
...
- [Stella can now lint your code and fix any errors](https://stella.chat/2024/05/22/linting.html).
  - Stella automatically lints and fixes after every LLM edit.
  - You can manually lint-and-fix files with `/lint` in the chat or `--lint` on the command line.
  - Stella includes built in basic linters for all supported tree-sitter languages.
  - You can also configure stella to use your preferred linter with `--lint-cmd`.
- Stella has additional support for running tests and fixing problems.
  - Configure your testing command with `--test-cmd`.
  - Run tests with `/test` or from the command line with `--test`.
  - Stella will automatically attempt to fix any test failures.
  

### Stella v0.35.0

- Stella now uses GPT-4o by default.
  - GPT-4o tops the [stella LLM code editing leaderboard](https://stella.chat/docs/leaderboards/) at 72.9%, versus 68.4% for Opus.
  - GPT-4o takes second on [stella's refactoring leaderboard](https://stella.chat/docs/leaderboards/#code-refactoring-leaderboard) with 62.9%, versus Opus at 72.3%.
...
- Added native support for [Deepseek models](https://stella.chat/docs/llms.html#deepseek) using `DEEPSEEK_API_KEY` and `deepseek/deepseek-chat`, etc rather than as a generic OpenAI compatible API.

### Stella v0.32.0

- [Stella LLM code editing leaderboards](https://stella.chat/docs/leaderboards/) that rank popular models according to their ability to edit code.
...
- [Stella is now also AI pair programming in your browser!](https://stella.chat/2024/05/02/browser.html) Use the `--browser` switch to launch an experimental browser based version of stella.
...
- Added [Gemini 1.5 Pro](https://stella.chat/docs/llms.html#free-models) as a recommended free model.
...
- Improved [model warnings](https://stella.chat/docs/llms.html#model-warnings) for unknown or unfamiliar models
...
- Added support for [directly connecting to Anthropic, Cohere, Gemini and many other LLM providers](https://stella.chat/docs/llms.html).
...
- New command line switches for working with popular models:
  ...
  - `--sonnet`
  - `--anthropic-api-key`
- Improved "whole" and "diff" backends to better support [Cohere's free to use Command-R+ model](https://stella.chat/docs/llms.html#cohere).
...
- Added support for new `gpt-4-turbo-2024-04-09` and `gpt-4-turbo` models.
  - Benchmarked at 61.7% on Exercism benchmark, comparable to `gpt-4-0613` and worse than the `gpt-4-preview-XXXX` models. See [recent Exercism benchmark results](https://stella.chat/2024/03/08/claude-3.html).
  - Benchmarked at 34.1% on the refactoring/laziness benchmark, significantly worse than the `gpt-4-preview-XXXX` models. See [recent refactor bencmark results](https://stella.chat/2024/01/25/benchmarks-0125.html).
  - Stella continues to default to `gpt-4-1106-preview` as it performs best on both benchmarks, and significantly better on the refactoring/laziness benchmark.
...
- Issue a warning if user adds too much code to the chat.
  - https://stella.chat/docs/faq.html#how-can-i-add-all-the-files-to-the-chat
- Vocally refuse to add files to the chat that match `.stellaignore`
...
- Made past articles part of https://stella.chat/blog/
...
- [Significantly reduced "lazy" coding from GPT-4 Turbo due to new unified diff edit format](https://stella.chat/docs/unified-diffs.html)
  - Score improves from 20% to 61% on new "laziness benchmark".
  - Stella now uses unified diffs by default for `gpt-4-1106-preview`.
...
- [Benchmarks for OpenAI's new 11/06 models](https://stella.chat/docs/benchmarks-1106.html)
- Streamlined [API for scripting stella, added docs](https://stella.chat/docs/faq.html#can-i-script-stella)
- Ask for more concise SEARCH/REPLACE blocks. [Benchmarked](https://stella.chat/docs/benchmarks.html) at 63.9%, no regression.
...
- [Improved repository map using tree-sitter](https://stella.chat/docs/repomap.html)
- Switched from "edit block" to "search/replace block", which reduced malformed edit blocks. [Benchmarked](https://stella.chat/docs/benchmarks.html) at 66.2%, no regression.
- Improved handling of malformed edit blocks targeting multiple edits to the same file. [Benchmarked](https://stella.chat/docs/benchmarks.html) at 65.4%, no regression.
- Bugfix to properly handle malformed `/add` wildcards.


### Stella v0.15.0

- Added support for `.stellaignore` file, which instructs stella to ignore parts of the git repo.
- New `--commit` cmd line arg, which just commits all pending changes with a sensible commit message generated by gpt-3.5.
- Added universal ctags and multiple architectures to the [stella docker image](https://stella.chat/docs/install/docker.html)
- `/run` and `/git` now accept full shell commands, like: `/run (cd subdir; ls)`
- Restored missing `--encoding` cmd line switch.

### Stella v0.14.2

- Easily [run stella from a docker image](https://stella.chat/docs/install/docker.html)
...
- Fixed /add bug in when stella launched in repo subdir
- Show models supported by api/key if requested model isn't available

### Stella v0.14.0

- [Support for Claude2 and other LLMs via OpenRouter](https://stella.chat/docs/faq.html#accessing-other-llms-with-openrouter) by @joshuavial
- Documentation for [running the stella benchmarking suite](https://github.com/Stella-AI/stella/tree/main/benchmark)
- Stella now requires Python >= 3.9


### Stella v0.13.0

- [Only git commit dirty files that GPT tries to edit](https://stella.chat/docs/faq.html#how-did-v0130-change-git-usage)
...
- Late-bind importing `sounddevice`, as it was slowing down stella startup
- Improved --foo/--no-foo switch handling for command line and yml config settings

### Stella v0.12.0

- [Voice-to-code](https://stella.chat/docs/usage/voice.html) support, which allows you to code with your voice.
- Fixed bug where /diff was causing crash.
- Improved prompting for gpt-4, refactor of editblock coder.
- [Benchmarked](https://stella.chat/docs/benchmarks.html) at 63.2% for gpt-4/diff, no regression.
...
- [Benchmarked](https://stella.chat/docs/benchmarks.html) at 53.4% for gpt-3.5/whole (no regression).
...
- Stronger GPT-3.5 prompt against skipping/eliding code in replies (51.9% [benchmark](https://stella.chat/docs/benchmarks.html), no regression)
...
- Added `/git` command to run git from inside stella chats.
- Use Meta-ENTER (Esc+ENTER in some environments) to enter multiline chat messages.
- Create a `.gitignore` with `.stella*` to prevent users from accidentally adding stella files to git.
- Check pypi for newer versions and notify user.
- Updated keyboard interrupt logic so that 2 ^C in 2 seconds always forces stella to exit.
- Provide GPT with detailed error if it makes a bad edit block, ask for a retry.
- Force `--no-pretty` if stella detects it is running inside a VSCode terminal.
- [Benchmarked](https://stella.chat/docs/benchmarks.html) at 64.7% for gpt-4/diff (no regression)


### Stella v0.9.0

- Support for the OpenAI models in [Azure](https://stella.chat/docs/faq.html#azure)
- Added `--show-repo-map`
...
- [Benchmarked](https://stella.chat/docs/benchmarks.html) at 53.8% for gpt-3.5-turbo/whole (no regression)

### Stella v0.8.3

- Added `--dark-mode` and `--light-mode` to select colors optimized for terminal background
- Install docs link to [NeoVim plugin](https://github.com/joshuavial/stella.nvim) by @joshuavial
...
- Bugfix so that stella throws an exception when OpenAI returns InvalidRequest
...
- Added [FAQ](https://stella.chat/docs/faq.html)
...
- [Benchmark comparing code editing in GPT-3.5 and GPT-4](https://stella.chat/docs/benchmarks.html)
...
- Fixed a bug to allow stella to edit files that contain triple backtick fences.
