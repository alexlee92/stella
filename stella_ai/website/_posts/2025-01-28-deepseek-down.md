...
If you're affected by these issues, several alternative providers offer access to DeepSeek V3. This article compares their performance on stella's polyglot benchmark to help you choose a reliable alternative.
...
You can use stella with OpenRouter like this:

```bash
# Set your API key using environment variables
export OPENROUTER_API_KEY=<your-key>
stella --model openrouter/deepseek/deepseek-chat

# Or use the --api-key command line option
stella --model openrouter/deepseek/deepseek-chat --api-key openrouter=<your-key>

# Or add it to .stella.conf.yml in your home directory or project root:
api-key:
  - openrouter=<your-key>
```
...
[stella's model settings](https://stella.chat/docs/config/adv-model-settings.html#model-settings).
Create a `.stella.model.settings.yml` file in your home directory or git project root with settings like this:
...
stella --model fireworks_ai/accounts/fireworks/models/deepseek-chat

# Or use the --api-key command line option
stella --model fireworks_ai/accounts/fireworks/models/deepseek-chat --api-key fireworks=<your-key>
...
stella --model openai/deepseek-ai/DeepSeek-V3

# Or use the --api-key command line option
stella --model openai/deepseek-ai/DeepSeek-V3 --api-key openai=<your-key>
...
stella --model ollama/deepseek-v3
```

It's important to provide model settings, especially the `num_ctx` parameter to
set the context window.
Ollama uses a 2k context window by default, which is very small for working with stella.
...
So if your context window is too small, you won’t get an explicit error. The biggest symptom will be that stella says it can’t see (some of) the files you added to the chat. That’s because ollama is silently discarding them because they exceed the context window.
...
You will need to properly configure stella to work with DeepSeek V3 when served
via other providers:

- Determine the `--model` name to use.
- Provide your API key to stella.
- Add model settings to `.stella.model.settings.yml`.


Adapt the `.stella.model.settings.yml` shown above for Fireworks. You will need to change the `name` field to match you chosen provider's model naming scheme.

See [Advanced model settings](https://stella.chat/docs/config/adv-model-settings.html#model-settings) for details about all stella model settings
