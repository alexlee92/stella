...
## Summary
The $6.32 cost reported to run the stella polyglot benchmark on
...
package (used by stella for LLM API connections) was not properly including reasoning tokens in 
...
Stella's own internal, correct pricing data was utilized during the benchmark.
...
Stella picked up this change April 28, 2025 when it upgraded its litellm dependency 
from v1.65.7 to v1.67.4.post1
in commit [9351f37](https://github.com/Stella-AI/stella/commit/9351f37).
That dependency change shipped on May 5, 2025 in stella v0.82.3.
...
So even though stella had correct per-token pricing, it did not have the correct token counts
used during the benchmark.
This resulted in an underestimate of the benchmark costs.

The incorrect litellm database entry does not appear to have affected the stella benchmark costs.
Stella maintains and uses its own database of costs for some models, and it contained
the correct pricing at the time of the benchmark.
Stella appears to have
loaded the correct cost data from its database and made use of it during the benchmark.

Every stella benchmark report contains the git commit hash of the stella repository state used to
run the benchmark.
The 
[benchmark run in question](https://github.com/Stella-AI/stella/blob/edbfec0ce4e1fe86735c915cb425b0d8636edc32/stella/website/_data/polyglot_leaderboard.yml#L814)
was built from 
commit [0282574](https://github.com/Stella-AI/stella/commit/0282574).
...
- Stella's internal model database correctly overrides the litellm database, which contained an incorrect token cost at the time.
- The correct pricing is loaded from stella's internal model database and produces similar (incorrect) costs as the original run.
- Updating stella's internal model database with an absurdly high token cost resulted in an appropriately high benchmark cost report, demonstrating that the internal database costs were in effect.

This specific build of stella was then updated with various versions of litellm using `git biset`
...
Below is the full timeline of git commits related to this issue in the stella and litellm repositories.
...
- 2025-04-04 19:54:45 UTC (Sat Apr 5 08:54:45 2025 +1300)
  - Correct value `"output_cost_per_token": 0.000010` for  `gemini/gemini-2.5-pro-preview-03-25` added to `stella/resources/model-metadata.json`
  - Commit [eda796d](https://github.com/Stella-AI/stella/commit/eda796d) in stella.
...
- 2025-04-12 04:55:50 UTC (2025-04-12-04-55-50 UTC)
  - Benchmark performed.
  - Stella repo hash [0282574 recorded in benchmark results](https://github.com/Stella-AI/stella/blob/7fbeafa1cfd4ad83f7499417837cdfa6b16fe7a1/stella/website/_data/polyglot_leaderboard.yml#L814), without a "dirty" annotation, indicating that the benchmark was run on a clean checkout of the stella repo at commit [0282574](https://github.com/Stella-AI/stella/commit/0282574).
  - Correct value `"output_cost_per_token": 0.000010` is in `stella/resources/model-metadata.json` at this commit [0282574](https://github.com/Stella-AI/stella/blob/0282574/stella/resources/model-metadata.json#L357).

- 2025-04-12 15:06:39 UTC (Apr 12 08:06:39 2025 -0700)
  - Benchmark results added to stella repo.
  - Commit [7fbeafa](https://github.com/Stella-AI/stella/commit/7fbeafa) in stella.
...
- 2025-04-28 14:53:20 UTC (Mon Apr 28 07:53:20 2025 -0700)
  - Stella upgraded its litellm dependency from v1.65.7 to v1.67.4.post1, which included the reasoning token count fix.
  - Commit [9351f37](https://github.com/Stella-AI/stella/commit/9351f37) in stella.
  - This dependency change shipped on May 5, 2025 in stella v0.82.3.
