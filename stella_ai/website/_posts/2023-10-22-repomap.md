...
excerpt: Tree-sitter allows stella to build a repo map that better summarizes large code bases.
...
To address these issues, stella
...
Stella now uses it to help GPT better comprehend, navigate
and edit code in larger repos.

*To code with GPT-4 using the techniques discussed here, just install [stella](https://stella.chat/docs/install.html).*
...
Stella also strives to reduce the manual work involved in
coding with AI.
So in an ideal world, we'd like stella to automatically
identify and provide the needed code context.

## Using a repo map to provide context

Stella sends a **repo map** to GPT along with
...
Mapping out the repo like this provides some key benefits:
  ...
  - If it needs to see more code, GPT can use the map to figure out by itself which files it needs to look at in more detail. GPT will then ask to see these specific files, and stella will automatically add them to the chat context.
...
[most popular programming languages](https://github.com/Stella-AI/grep-ast/blob/main/grep_ast/parsers.py).
...
Stella uses all of these definitions and references to
...
[ctags based map](https://stella.chat/docs/ctags.html)
that stella originally used.
...
- Thanks to `py-tree-sitter-languages`, we get full support for many programming languages via a python package that's automatically installed as part of the normal `python -m pip install -U stella-chat`.
...
- Tree-sitter integration is a key enabler for future work and capabilities for stella.
...
Right now, stella relies on the user to specify which source files
will need to be modified to complete their request.
Users manually "add files to the chat" using stella's `/add` command,
...
just install [stella](https://stella.chat/docs/install.html).

## Credits

Stella uses
[modified versions of the tags.scm files](https://github.com/Stella-AI/stella/tree/main/stella/queries)
from these
