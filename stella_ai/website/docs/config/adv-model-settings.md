...
In most cases, you can safely ignore stella's warning about unknown context
window size and model costs.

{: .note }
Stella never *enforces* token limits, it only *reports* token limit errors
from the API provider.
You probably don't need to
configure stella with the proper token limits
for unusual models.

But, you can register context window limits and costs for models that aren't known
to stella. Create a `.stella.model.metadata.json` file in one of these locations:

- Your home directory.
- The root if your git repo.
- The current directory where you launch stella.
...
in the `.stella.model.metadata.json` file.
...
Stella has a number of settings that control how it works with
different models.
...
a model that stella doesn't know about.


### Configuration file locations

You can override or add settings for any model by creating a `.stella.model.settings.yml` file in one of these locations:
...
You can use the special model name `stella/extra_params` to define 
...
`stella/extra_params` settings taking precedence for any direct conflicts.
...
[models.py](https://github.com/Stella-AI/stella/blob/main/stella/models.py)
file for more details about all of the model setting that stella supports.
...
<!--[[[cog
from stella.models import get_model_settings_as_yaml
cog.out("```yaml\n")
