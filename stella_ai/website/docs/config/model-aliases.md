...
You can define aliases when launching stella using the `--alias` option:

```bash
stella --alias "fast:gpt-4o-mini" --alias "smart:o3-mini"
```
...
Of course,
you can also define aliases in your [`.stella.conf.yml` file](https://stella.chat/docs/config/stella_conf.html):
...
```bash
stella --model fast  # Uses gpt-4o-mini
stella --model smart  # Uses o3-mini
```
...
Stella includes some built-in aliases for convenience:

<!--[[[cog
import cog
from stella.models import MODEL_ALIASES
