...
Stella is configured to work with most popular reasoning models out of the box. 
You can use them like this:

```bash
# Sonnet uses a thinking token budget
stella --model sonnet --thinking-tokens 8k

# o3-mini uses low/medium/high reasoning effort
stella --model o3-mini --reasoning-effort high

# R1 doesn't have configurable thinking/reasoning
stella --model r1
```

Inside the stella chat, you can use `/thinking-tokens 4k` or `/reasoning-effort low` to change
...
if you're configuring stella to work with a lesser known reasoning model or one served
via an unusual provider.

## Reasoning settings

Different models support different reasoning settings. Stella provides several ways to control reasoning behavior:
...
Not all models support these two settings. Stella uses the 
...
If you try to use a setting that a model doesn't explicitly support, Stella will warn you:
...
This configuration:
1. Tells Stella that the model accepts the `reasoning_effort` setting
2. Indicates the model does NOT accept `thinking_tokens` (since it's not listed)
3. Causes Stella to ignore any `--thinking-tokens` value passed for this model
...
1. Force Stella to apply all settings passed via command line
...
`<think>...</think>` tags, so stella's settings
...
Stella will display the thinking/reasoning output, 
but it won't be used for file editing instructions, added to the chat history, etc.
Stella will rely on the non-thinking output for instructions on how to make code changes, etc.
...
Stella is configured to work properly with popular models
...
Include settings for your new provider in `.stella.model.settings.yml` file
...
[existing model setting configuration entries](https://github.com/Stella-AI/stella/blob/main/stella/resources/model-settings.yml)
...
Note that stella already has these settings pre-configured, but they
