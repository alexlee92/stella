---
nav_order: 90
description: Frequently asked questions about stella.
---
...
Stella will automatically give the LLM a bunch of additional context about
...
- Use a wildcard when you launch stella: `stella src/*.py`
- Use a wildcard with the in-chat `/add` command: `/add src/*.py`
- Give the `/add` command a directory name and it will recursively add every file under that dir: `/add src`

## Can I use stella in a large (mono) repo?

Stella will work in any size repo, but is not optimized for quick
...
To get the best results from stella you want to 
...
This will tell stella to ignore the repo outside of the
directory you start in.

You can also create a `.stellaignore` file to tell stella
...
you could create a `.stellaignore` file with:
...
You can use `--stellaignore <filename>` to name a specific file
...
## Can I use stella with multiple git repos at once?

Currently stella can only work with one repo at a time.

There are some things you can try if you need to work with
multiple interrelated repos:

- You can run stella in repo-A where you need to make a change
and use `/read` to add some files read-only from another repo-B.
This can let stella see key functions or docs from the other repo.
- You can run `stella --show-repo-map > map.md` within each
repo to create repo maps.
You could then run stella in repo-A and 
use `/read ../path/to/repo-B/map.md` to share
a high level map of the other repo.
- You can use stella to write documentation about a repo.
Inside each repo, you could run `stella docs.md`
and work with stella to write some markdown docs.
Then while using stella to edit repo-A
you can `/read ../path/to/repo-B/docs.md` to
read in those docs from the other repo.
- In repo A, ask stella to write a small script that demonstrates
the functionality you want to use in repo B.
Then when you're using stella in repo B, you can 
`/read` in that script.

## How do I turn on the repository map?

Depending on the LLM you are using, stella may launch with the repo map disabled by default:
...
If you would like to force it on, you can run stella with `--map-tokens 1024`.

## How do I include the git history in the context?

When starting a fresh stella session, you can include recent git history in the chat context. This can be useful for providing the LLM with information about recent changes. To do this:
...
Remember, the chat history already includes recent changes made during the current session, so this tip is most useful when starting a new stella session and you want to provide context about recent work.

You can also use stella to review PR branches:
...
And of course you can prepare diff output outside of stella and provide it as
a file for stella to read:

```
$ git diff -C10 v1..v2 > v1-v2-changes.diff
$ stella --read v1-v2-changes.diff

Stella v0.77.2.dev+import
...
## How can I run stella locally from source code?

To run the project locally, follow these steps:

```
# Clone the repository
git clone git@github.com:Stella-AI/stella.git
...
# Install stella in editable/development mode, 
# so it runs from the latest copy of these source files
python -m pip install -e .

# Run the local version of stella
python -m stella
```



## Can I change the system prompts that stella uses?

The most convenient way to add custom instructions is to use a
[conventions file](https://stella.chat/docs/usage/conventions.html).

But, stella is set up to support different actual system prompts and edit formats
in a modular way. If you look in the `stella/coders` subdirectory, you'll
...
this document about
[benchmarking GPT-3.5 and GPT-4 on code editing](https://stella.chat/docs/benchmarks.html)
...
When experimenting with coder backends, it helps to run stella with `--verbose --no-pretty` so you can see
all the raw information being sent to/from the LLM in the conversation.

You can also refer to the
[instructions for installing a development version of stella](https://stella.chat/docs/install/optional.html#install-the-development-version-of-stella).

## What LLMs do you use to build stella?

Stella writes a lot of its own code, usually about 70% of the new code in each
release.
People often ask which LLMs I use with stella, when writing stella.
Below is a table showing the models I have used recently,
extracted from the 
[public log](https://github.com/stella-ai/stella/blob/main/stella/website/assets/sample-analytics.jsonl)
of my
[stella analytics](https://stella.chat/docs/more/analytics.html).
...
## How are the "stella wrote xx% of code" stats computed?

[Stella is tightly integrated with git](/docs/git.html) so all
of stella's code changes are committed to the repo with proper attribution.
The 
[stats are computed](https://github.com/Stella-AI/stella/blob/main/scripts/blame.py)
by doing something like `git blame` on the repo,
and counting up who wrote all the new lines of code in each release.
Only lines in source code files are counted, not documentation or prompt files.

## Why did stella ignore/discard its proposed edits after it asked to add a new file to the chat?

If stella prompts you to add a new file to the chat and you say yes,
...
So stella re-submits the original request in this situation.

## Why does stella sometimes stop highlighting code in its replies?

Stella displays the markdown responses that are coming back from the LLM.
...
stella needs to tell the LLM to use a different set of fences.
Otherwise, the LLM can't safely include your code's triple backticks
inside the code blocks that it returns with edits.
Stella will use fences like `<source>...</source>` in this case.

A side effect of this is that the code that stella outputs may no
...
Stella goes to some effort to prompt the model to use the language that is configured
for your system.
...
You can explicitly set the language that stella tells the model to use with
`--chat-language <language>`.
But the LLM may not comply.

## Can I share my stella chat transcript?

Yes, you can now share stella chat logs in a pretty way.

1. Copy the markdown logs you want to share from `.stella.chat.history.md` and make a github gist. Or publish the raw markdown logs on the web any way you'd like.

   ```
   https://gist.github.com/Stella-AI/2087ab8b64034a078c0a209440ac8be0
   ```

2. Take the gist URL and append it to:

   ```
   https://stella.chat/share/?mdurl=
   ```

This will give you a URL like this, which shows the chat history like you'd see in a terminal:

```
https://stella.chat/share/?mdurl=https://gist.github.com/Stella-AI/2087ab8b64034a078c0a209440ac8be0
```

## Can I edit files myself while stella is running?

Yes. Stella always reads the latest copy of files from the file
system when you send each message.

While you're waiting for stella's reply to complete, it's probably unwise to
edit files that you've added to the chat.
Your edits and stella's edits might conflict.

## What is Stella AI LLC?

Stella AI LLC is the company behind the stella AI coding tool.
Stella is 
[open source and available on GitHub](https://github.com/Stella-AI/stella)
under an 
[Apache 2.0 license](https://github.com/Stella-AI/stella/blob/main/LICENSE.txt).

## Can I Script Stella?

Yes. You can script stella via the command line or python. See more from here: [Scripting stella](https://stella.chat/docs/scripting.html)
