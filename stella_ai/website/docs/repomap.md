...
nav_order: 300
description: Stella uses a map of your git repository to provide code context to LLMs.
---
...
Here's a part of
the repo map of stella's repo, for
[base_coder.py](https://github.com/Stella-AI/stella/blob/main/stella/coders/base_coder.py)
and
[commands.py](https://github.com/Stella-AI/stella/blob/main/stella/commands.py)
:

```
stella/coders/base_coder.py:
⋮...
│class Coder:
│    abs_fnames = None
⋮...
...
Mapping out the repo like this provides some key benefits:
  ...
  - If it needs to see more code, the LLM can use the map to figure out which files it needs to look at. The LLM can ask to see these specific files, and stella will offer to add them to the chat context.

## Optimizing the map

Of course, for large repositories even just the repo map might be too large
for the LLM's context window.
Stella solves this problem by sending just the **most relevant**
...
Stella optimizes the repo map by
...
Stella adjusts the size of the repo map dynamically based on the state of the chat. It will usually stay within that setting's value. But it does expand the repo map
significantly at times, especially when no files have been added to the chat and stella needs to understand the entire repo as best as possible.
...
Please check the
[repo map article on stella's blog](https://stella.chat/2023/10/22/repomap.html)
for more information on stella's repository map
