...
Stella can connect to GitHub Copilot’s LLMs because Copilot exposes a standard **OpenAI-style**
endpoint at:
...
Each returned ID can be used with stella by **prefixing it with `openai/`**:

```bash
stella --model openai/gpt-4o
# or
stella --model openai/claude-3.7-sonnet-thought
```
...
## Optional config file (`~/.stella.conf.yml`)
...
* Calls made through stella are billed through your Copilot subscription  
  (stella will still print *estimated* costs).
* The Copilot docs explicitly allow third-party “agents” that hit this API – stella is playing by
  the rules.
* Stella talks directly to the REST endpoint—no web-UI scraping or browser automation.
