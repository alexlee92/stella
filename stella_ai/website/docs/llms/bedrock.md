...
Stella can connect to models provided by Amazon Bedrock.
To configure Stella to use the Amazon Bedrock API, you need to set up your AWS credentials.
...
Once your AWS credentials are set up, you can run Stella with the `--model` command line switch, specifying the Bedrock model you want to use:

```bash
# Change directory into your codebase
cd /to/your/project

stella --model bedrock/anthropic.claude-3-5-sonnet-20240620-v1:0
```

Sometimes it seems to help if you prefix the model name with "us.":

```bash
stella --model bedrock/us.anthropic.claude-3-5-sonnet-20240620-v1:0
```
...
```bash
stella --list-models bedrock/
```

Make sure you have access to these models in your AWS account before attempting to use them with Stella.

## Install boto3
You may need to install the `boto3` package.

```bash
# If you installed with stella-install or `uv tool`
uv tool run --from stella-chat pip install boto3

# Or with pipx...
pipx inject stella-chat boto3
