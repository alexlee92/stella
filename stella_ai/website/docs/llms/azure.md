...
Stella can connect to the OpenAI models on Azure.
...
Start working with stella and Azure on your codebase:

```bash
# Change directory into your codebase
cd /to/your/project

stella --model azure/<your_model_deployment_name>

# List models available from Azure
stella --list-models azure/
```

Note that stella will also use environment variables
like `AZURE_OPENAI_API_xxx`.

The `stella --list-models azure/` command will list all models that stella supports through Azure, not the models that are available for the provided endpoint.
