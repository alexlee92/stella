...
Stella uses the [litellm](https://docs.litellm.ai/docs/providers) package
to connect to hundreds of other models.
You can use `stella --model <model-name>` to use any supported model.

To explore the list of supported models you can run `stella --list-models <model-name>`
with a partial model name.
If the supplied name is not an exact match for a known model, stella will
return a list of possible matching models.
For example:

```
$ stella --list-models turbo
...
when working with models that stella is not familiar with.

## LiteLLM

Stella uses the LiteLLM package to connect to LLM providers.
