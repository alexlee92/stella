...
Stella is available as 2 docker images:

- `paulgauthier/stella` installs the stella core, a smaller image that's good to get started quickly.
- `paulgauthier/stella-full` installs stella will all the optional extras.
...
launch the stella core container.

### Stella core 

```
docker pull paulgauthier/stella
docker run -it --user $(id -u):$(id -g) --volume $(pwd):/app paulgauthier/stella --openai-api-key $OPENAI_API_KEY [...other stella args...]
```

### Full version

```
docker pull paulgauthier/stella-full
docker run -it --user $(id -u):$(id -g) --volume $(pwd):/app paulgauthier/stella-full --openai-api-key $OPENAI_API_KEY [...other stella args...]
```
...
docker container.
Given that, you need to be in the root of your git repo for stella to be able to
