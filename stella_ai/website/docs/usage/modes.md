...
Stella has a few different chat modes:

- `code` - Stella will make changes to your code to satisfy your requests.
- `ask` - Stella will discuss your code and answer questions about it, but never make changes.
- `architect` - Like code mode, stella will change your files. An architect model will propose changes and an editor model will translate that proposal into specific file edits.
- `help` - Stella will answer questions about stella: usage, configuration, troubleshooting, etc.

By default, stella starts in "code" mode. As you are talking, you can
...
Or you can launch stella in one of the modes with the `--chat-mode <mode>` switch.
There is also a special shortcut `--architect` to launch in `--chat-mode architect`.

The stella prompt will indicate the active mode:
...
Use ask mode to discuss what you want to do, get suggestions or options from stella
and provide feedback on the approach.
Once stella understands the mission, switch to code mode to have it start editing
your files.
...
have stella execute on the plan you've been discussing.
...
When you are in architect mode, stella sends your requests to two models:
...
2. Stella then sends another request to an "editor model",
asking it to turn the architect's proposal into specific file editing instructions.
Stella has built in defaults to select an editor model based on your main model.
...
The editor model uses one of stella's edit formats to let the LLM
edit source files.
Stella will pick a suitable edit format by default,
...
See this article on 
[stella's architect/editor mode](/2024/09/26/architect.html)
for more details.
...
#### /help How do I use stella with ollama?

Run `stella --model ollama/<ollama-model>`.
See these docs for more info: https://stella.chat/docs/llms/ollama.html
