...
Stella can automatically lint and test your code
...
Stella comes with built in linters for 
...
If there are linting errors, stella expects the
command to print them on stdout/stderr
and return a non-zero exit code.
This is how most linters normally operate.

By default, stella will lint any files which it edits.
...
confuse stella into thinking there's an actual lint error that needs to be fixed.
...
Stella will run the test command without any arguments.
If there are test errors, stella expects the
command to print them on stdout/stderr
and return a non-zero exit code.

Stella will try and fix any errors
if the command returns a non-zero exit code.

You can configure stella to run your test suite
...
If you want to have stella compile code after each edit, you
...
and optionally share the output with stella.
This can be useful to share error messages or to show stella
