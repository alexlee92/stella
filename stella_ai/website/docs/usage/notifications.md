...
parent: Usage
nav_order: 760
description: Stella can notify you when it's waiting for your input.
---

# Notifications

Stella can notify you when it's done working and is
...
When enabled, stella will notify you when the LLM has finished generating a response and is waiting for your input.

## OS-Specific Notifications

Stella automatically detects your operating system and uses an appropriate notification method:
...
```bash
stella --notifications-command "your-custom-command"
```

For example, on macOS you might use:

```bash
stella --notifications-command "say 'Stella is ready'"
```
...
```bash
stella --notifications-command "apprise -b 'Stella is ready' 'slack://your-slack-webhook-token'"
```

or Discord
```bash
stella --notifications-command "apprise -b 'Stella is ready' 'discord://your-discord-webhook-token'"
```

or even to your phone via Pushbullet
```bash
stella --notifications-command "apprise -b 'Stella is ready' 'pbul://your-pushbullet-access-token'"
```
...
```
STELLA_NOTIFICATIONS=true
STELLA_NOTIFICATIONS_COMMAND=your-custom-command
```
