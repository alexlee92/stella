...
parent: Usage
nav_order: 50
description: Control stella with in-chat commands like /add, /model, etc.
---
...
Stella supports commands from within the chat, which all start with `/`.

<!--[[[cog
from stella.commands import get_help_md
cog.out(get_help_md())
]]]-->

|Command|Description|
|:------|:----------|
| **/add** | Add files to the chat so stella can edit them or review them in detail |
...
| **/help** | Ask questions about stella |
...
| **/undo** | Undo the last git commit if it was done by stella |
...
It's always safe to use Control-C to interrupt stella if it isn't providing a useful response. The partial response remains in the conversation, so you can refer to it when you reply to the LLM with more information or direction.
...
To use vi/vim keybindings, run stella with the `--vim` switch.
