...
parent: Usage
nav_order: 800
description: Tell stella to follow your coding conventions when it works on your code.
---
...
The easiest way to do that with stella is to simply create
...
and then we can add it to the stella chat, along with the file(s)
that we want to edit.

It's best to load the conventions file with `/read CONVENTIONS.md` 
or `stella --read CONVENTIONS.md`. 
This way it is marked as read-only, and cached if prompt caching
is enabled.

## Community contributed conventions

You can check the [stella conventions repository](https://github.com/Stella-AI/conventions)
to find or contribute conventions files.

## Always load conventions

You can also configure stella to always load your conventions file
in the [`.stella.conf.yml` config file](https://stella.chat/docs/config/stella_conf.html):
...
> $ stella --read CONVENTIONS.md useragent.py  
>  
> Stella v0.24.2-dev  
