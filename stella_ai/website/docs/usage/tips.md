...
parent: Usage
nav_order: 25
description: Tips for AI pair programming with stella.
---
...
Stella can often figure out which files to edit all by itself, but the most efficient approach is for you to add the files to the chat.
...
Stella uses a [map of your entire git repo](https://stella.chat/docs/repomap.html)
...
- If stella is hopelessly stuck,
just code the next step yourself and try having stella code some more after that.
Take turns and pair program with stella.

## Creating new files

If you want stella to create a new file, add it to the repository first with `/add <file>`.
This way stella knows this file exists and will write to it. 
Otherwise, stella might write the changes to an existing file.
...
to share the error output with the stella.
Or just paste the errors into the chat. Let the stella figure out how to fix the bug.

If test are failing, use the [`/test` command](lint-test.html)
to run tests and
share the error output with the stella.
...
and stella will scrape and read it. For example: `Add a submit button like this https://ui.shadcn.com/docs/components/button`. 
- Use the [`/read` command](commands.html) to read doc files into the chat from anywhere on your filesystem.
- If you have coding conventions or standing instructions you want stella to follow, consider using a [conventions file](conventions.html).

## Interrupting & inputting

Use Control-C to interrupt stella if it isn't providing a useful response. The partial response remains in the conversation, so you can refer to it when you reply with more information or direction.
