...
parent: More info
nav_order: 480
description: Stella can handle "infinite output" from models that support prefill.
---
...
Stella is able to work around this limit with models that support
"prefilling" the assistant response.
When you use stella with a model that supports prefill, you will see
...
When stella is collecting code edits from a model and
it hits the output token limit,
stella simply initiates another LLM request with the partial
response prefilled.
...
Stella supports "infinite output" for models that support "prefill",
such as:
