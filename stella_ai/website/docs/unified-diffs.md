...
Stella now asks GPT-4 Turbo to use
...
Stella's new "laziness" benchmark suite 
...
- **GPT-4 Turbo only scored 20% as a baseline** using stella's existing "SEARCH/REPLACE block" edit format. It outputs "lazy comments" on 12 of the tasks.
- **Stella's new unified diff edit format raised the score to 61%**. Using this format reduced laziness by 3X, with GPT-4 Turbo only using lazy comments on 4 of the tasks.
...
- **The June GPT-4's baseline was 26%** using stella's existing "SEARCH/REPLACE block" edit format.
- **Stella's new unified diff edit format raised June GPT-4's score to 59%**. 
...
Stella's new unified diff editing format
...
numerous variations on stella's existing editing formats,
...
stella's new editing format and refactoring benchmark.
...
The design and implementation of stella's new unified diff editing format
...
Stella's [previous benchmark results](https://stella.chat/docs/benchmarks.html) made
...
Stella tells GPT not to include line numbers,
...
Stella's system prompt encourages
...
When a patch fails, stella needs to ask GPT for a corrected version of the diff.
...
Stella tries to be very flexible when applying diffs,
in order to handle defects.
If a hunk doesn't apply cleanly, stella uses a number of strategies:
...
**Experiments where flexible patching is disabled show a 9X increase in editing errors** on stella's original Exercism benchmark.

## Refactoring benchmark

Stella has long used a
[benchmark suite based on 133 Exercism python exercises](https://stella.chat/2023/07/02/benchmarks.html).
...
[9 popular open source python repositories](https://github.com/Stella-AI/refactor-benchmark)
...
A [simple python AST scanning script](https://github.com/Stella-AI/stella/blob/main/benchmark/refactor_tools.py)
...
[benchmark suite that provokes, detects and quantifies GPT coding laziness](https://github.com/Stella-AI/refactor-benchmark).
...
stella's new unified diff format seems
...
when originally building stella.
...
incorporated into stella provide
...
stella's simple, high level style of unified diffs.
