...
Stella will report an error **if a model responds** indicating that
...
For more info: https://stella.chat/docs/token-limits.html
```

{: .note }
Stella never *enforces* token limits, it only *reports* token limit errors
from the API provider.
The token counts that stella reports are *estimates*.
...
It's best to only add the files that stella will need to *edit*
...
as 4k tokens.
If you ask stella to make a large change that affects a lot
...
Stella has been well tested when directly connecting to 
major 
[LLM provider cloud APIs](https://stella.chat/docs/llms.html).
For serving local models, 
[Ollama](https://stella.chat/docs/llms/ollama.html) is known to work well with stella.

Try using stella without an API proxy server
