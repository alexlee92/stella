...
from stella_ai import __version__, models, prompts, urls, utils
from stella_ai.analytics import Analytics
from stella_ai.commands import Commands
from stella_ai.exceptions import LiteLLMExceptions
from stella_ai.history import ChatSummary
from stella_ai.io import ConfirmGroup, InputOutput
from stella_ai.linter import Linter
from stella_ai.llm import litellm
from stella_ai.models import RETRY_TIMEOUT
from stella_ai.reasoning_tags import (
    REASONING_TAG,
...
from stella_ai.repomap import RepoMap
from stella_ai.run_cmd import run_cmd
from stella_ai.utils import format_content, format_messages, format_tokens, is_image_file
from stella_ai.waiting import WaitingSpinner
...
class Coder:
    ...
    repo = None
    last_stella_commit_hash = None
    stella_edited_files = None
    ...
    def create(
        ...
        **kwargs,
        ...
        import stella_ai.coders as coders
        ...
        if from_coder:
            ...
            update = dict(
                ...
                stella_commit_hashes=from_coder.stella_commit_hashes,
    ...
    def get_announcements(self):
        lines = []
        lines.append(f"Stella v{__version__}")
        ...
        if self.repo:
            ...
            if num_files > 1000:
                lines.append(
                    "Warning: For large repos, consider using --subtree-only and .stellaignore"
                )
    ...
    def __init__(
        ...
        if stella_commit_hashes:
            self.stella_commit_hashes = stella_commit_hashes
        else:
            self.stella_commit_hashes = set()
        ...
        for fname in fnames:
            ...
            if self.repo and self.repo.ignored_file(fname):
                self.io.tool_warning(f"Skipping {fname} that matches stellaignore spec.")
                continue
    ...
    def warm_cache(self, chunks):
        ...
        delay = 5 * 60 - 5
        delay = float(os.environ.get("STELLA_CACHE_KEEPALIVE_DELAY", delay))
    ...
    def send_message(self, inp):
        ...
        if edited:
            self.stella_edited_files.update(edited)
    ...
    def auto_commit(self, edited, context=None):
        ...
        try:
            res = self.repo.commit(fnames=edited, context=context, stella_edits=True, coder=self)
            if res:
    ...
    def show_auto_commit_outcome(self, res):
        commit_hash, commit_message = res
        self.last_stella_commit_hash = commit_hash
        self.stella_commit_hashes.add(commit_hash)
        self.last_stella_commit_message = commit_message
        if self.show_diffs:
    ...
    def show_undo_hint(self):
        ...
        if self.commit_before_message[-1] != self.repo.get_head_commit_sha():
            self.io.tool_output("You can use /undo to undo and discard each stella commit.")
